...
void slacpy_full_kernel_batched(
    int m, int n,
    float const * const *dAarray, icla_int_t Ai, icla_int_t Aj, int ldda,
    float **dBarray, icla_int_t Bi, icla_int_t Bj, int lddb )
{
    ...
    float **dBarray, icla_int_t* lddb )
{
    ...
    if(blockIdx.x >= icla_ceildiv(my_m, BLK_X)) return;
    if(blockIdx.y >= icla_ceildiv(my_n, BLK_Y)) return;
