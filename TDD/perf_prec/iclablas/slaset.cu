...
const icla_int_t max_blocks = 65535;
    ...
    float *A, int lda )
{
    ...
    bool full = (iby + BLK_Y <= n && (ind >= iby + BLK_Y || ind + BLK_X <= iby || ICLA_S_EQUAL( offdiag, diag )));
...
/***************************************************************************//**
    ...
    uplo    icla_uplo_t
      ...
      -     = IclaUpper:      Upper triangular part
      -     = IclaLower:      Lower triangular part
      -     = IclaFull:       All of the matrix dA
    ...
    dA      REAL array, dimension (LDDA,N)
            ...
            If UPLO = IclaUpper, only the upper triangle or trapezoid is accessed;
            if UPLO = IclaLower, only the lower triangle or trapezoid is accessed.
    ...
    icla_queue_t queue)
{
    ...
    assert( BLK_X == BLK_Y );
    const icla_int_t super_NB = max_blocks*BLK_X;
    dim3 super_grid( icla_ceildiv( m, super_NB ), icla_ceildiv( n, super_NB ) );
    ...
    if (uplo == IclaLower) {
        ...
        if ( m == ldda &&
             ICLA_S_EQUAL( offdiag, ICLA_S_ZERO ) &&
             ICLA_S_EQUAL( diag,    ICLA_S_ZERO ) )
        {
            size_t size = m*n;
    ...
    icla_int_t batchCount, icla_queue_t queue)
{
    ...
    icla_int_t max_batchCount = queue->get_maxBatch();

    for(icla_int_t i = 0; i < batchCount; i+=max_batchCount) {
        icla_int_t ibatch = min(max_batchCount, batchCount-i);
        dim3 grid( icla_ceildiv( m, BLK_X ), icla_ceildiv( n, BLK_Y ), ibatch );
    ...
    icla_int_t batchCount, icla_queue_t queue)
{
    ...
    iclablas_slaset_internal_batched(
    ...
    icla_int_t batchCount, icla_queue_t queue)
{
    ...
    for(icla_int_t i = 0; i < batchCount; i+=max_batchCount) {
        icla_int_t ibatch = min(max_batchCount, batchCount-i);
        dim3 grid( icla_ceildiv( max_m, BLK_X ), icla_ceildiv( max_n, BLK_Y ), ibatch );
